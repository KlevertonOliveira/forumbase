...
import { calculateAnswerTitleValue } from '../../helpers/other/calculateAnswerTitleValue';
...
const Post: NextPage = () => {
  ...
  async function handleCreateAnswer({ content }: AnswerForm) {
    ...
    try {
      ...
      feedbackContent = 'Your answer has been successfully added!';
    }
  ...
  const currentPageAnswers = Object.keys(post).length > 0 ? post.answers!.sort(compareTimestamps).slice(pagesVisited, pagesVisited + answersPerPage) : [];
                ...
                <Box as='section'>
...

                  {
                    ...
                    <Stack direction='column' spacing={8}>
                      {
                        currentPageAnswers.sort(compareTimestamps).map((answer: TAnswer, index) =>
                          <PostInteractiveArea
                            key={answer.id!}
                            type='answer'
                            postId={postKey}
                            answerId={answer.id!}
                            title={`Answer ${calculateAnswerTitleValue(totalAnswers, currentSelectedPage, index)}`}
